...
TTS_VERSION_TAG = "v15-pausas-naturales-2026-07-02"
...
def _texto_a_ssml(texto: str) -> str:
    ...
    def _format_pct(m):
        # FASE 31.1: en español los decimales se leen con COMA ("cero coma cuatro")
    ...
    # FASE 31.1: proteger puntos suspensivos ANTES de las pausas por punto
    # (antes se convertían en '.. . <break>' y sonaban como tartamudeo)
    t = t.replace('\u2026', '...')
    t = t.replace('... ', ' <break time="600ms"/> ')
    if t.endswith('...'):
        t = t[:-3] + ' <break time="400ms"/>'
    ...
    t = t.replace('? ', '? <break time="500ms"/> ')
    t = t.replace('! ', '! <break time="500ms"/> ')
    # FASE 31.1: pausas adicionales de naturalidad
    t = t.replace(') ', ') <break time="200ms"/> ')
...
def _llamar_google_tts(texto: str) -> Optional[bytes]:
    ...
    if USE_SSML:
        try:
            # ═══ FASE 31.1 — CAUSA RAÍZ DE "NO RESPETA PAUSAS" ═══
            # Google TTS rechaza inputs > 5000 bytes. El SSML EXPANDE mucho el
            # texto (breaks ~26 chars c/u, <sub>, escapes). Con respuestas
            # largas el SSML superaba el límite → Google lo RECHAZABA y el
            # código caía a TEXTO PLANO: sin pausas ni pronunciaciones.
            # FIX: recortar el texto en el último punto hasta que el SSML
            # quepa — así el audio SIEMPRE conserva pausas y diccionario.
            texto_ssml = texto
            ssml = _texto_a_ssml(texto_ssml)
            _recortes = 0
            while len(ssml.encode('utf-8')) > 4800 and len(texto_ssml) > 400:
                corte = texto_ssml.rfind('. ', 0, int(len(texto_ssml) * 0.9))
                if corte < 300:
                    corte = int(len(texto_ssml) * 0.85)
                texto_ssml = texto_ssml[:corte + 1].rstrip()
                ssml = _texto_a_ssml(texto_ssml)
                _recortes += 1
            if _recortes:
                print(f"✂️ [TTS-SSML] Texto acotado ({_recortes} pasos) para respetar límite 5000 bytes — pausas intactas")
                logger.info(f"✂️ [TTS-SSML] acotado x{_recortes} → {len(ssml.encode('utf-8'))} bytes")
            print(f"🔊 [TTS-SSML] {len(ssml.encode('utf-8'))} bytes | preview: {ssml[:160]}...")
            payload_ssml = {
                "input": {"ssml": ssml},
            ...
            r = requests.post(url, json=payload_ssml, timeout=30)
