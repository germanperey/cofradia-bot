...
#   - es-US-Neural2-C: masculina (fue default; v18 la retira — la voz del bot es FEMENINA siempre)
...
VOICE_NAME     = os.getenv("GOOGLE_TTS_VOICE", "es-US-Neural2-A")  # v18: respaldo TAMBIÉN femenino
...
SPEAKING_RATE  = float(os.getenv("GOOGLE_TTS_RATE",  "0.90"))  # v18: más pausada (Germán: "habla muy rápido")
# Pitch 0.0 = tono natural de la voz Neural2-C masculina. No se altera.
PITCH          = float(os.getenv("GOOGLE_TTS_PITCH", "1.5"))  # v18: tono femenino cálido en el respaldo
...
TTS_VERSION_TAG = "v31.53-catalina-numeros-2026-07-23"  # v18: invalida audios masculinos cacheados
...
def _limpiar_texto(texto: str) -> str:
    # ═══ FASE 31.53 (v18): NÚMEROS LEÍDOS COMO CANTIDAD, NO DÍGITO A DÍGITO ═══
    # Problema reportado por Germán: "30 000" o "1.234.567" se leían por
    # dígitos. Causa: separadores de miles (punto chileno, espacios normales
    # y finos U+202F/U+00A0) parten el número. Solución: colapsarlos ANTES
    # de cualquier motor (esta función alimenta a Catalina Y a Google).
    # Además "1991-2001" → "1991 a 2001" para lectura natural de rangos.
    texto = re.sub(r"\b((?:1[89]|20)\d{2})\s*[-–]\s*((?:1[89]|20)\d{2})\b",
                   r"\1 a \2", texto)
    for _ in range(4):  # "1.234.567" necesita pasadas sucesivas
        nuevo = re.sub(r"(?<=\d)[\.\u202f\u00a0 ](?=\d{3}(?!\d))", "", texto)
        if nuevo == texto:
            break
        texto = nuevo
